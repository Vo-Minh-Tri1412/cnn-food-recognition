...
class ColabNotebookTests(unittest.TestCase):
    ...
    def test_each_model_writes_to_its_own_drive_run(self):
        ...
        self.assertIn('DRIVE_ROOT / "init_checkpoints" / "dish_classifier_production_20260620.pt"', self.code)
        self.assertIn('CLS_RUN_ROOT / "dish_classifier_candidate.pt"', self.code)
        self.assertNotIn('CLS_MODEL_OUT = DRIVE_MODELS / "dish_classifier.pt"', self.code)
