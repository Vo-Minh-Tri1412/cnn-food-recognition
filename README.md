...
### Build Weighted Training Dataset

After review, build `data/classification/{train,val,test}` from old curated candidates plus newly reviewed images:

```powershell
.\.venv\Scripts\python.exe scripts\17_build_weighted_classification_dataset.py --clear
```

Defaults intentionally trust the old curated candidates more:

```text
old source      = latest data/downloads/merge_batches/merge_*/processed
reviewed source = latest data/downloads/external_staging/external_*/reviewed
old train weight = 4
reviewed train weight = 1
reviewed cap = 2x old unique images per class
```

Only the train split is weighted. Validation and test keep one copy per unique image so evaluation is not inflated.

`--clear` only removes files generated by this script (`old_*`, `reviewed_*`) and preserves images you manually add to `data/classification`. Use `--clear-all` only if you explicitly want a full rebuild and are sure there are no manual files to keep.
